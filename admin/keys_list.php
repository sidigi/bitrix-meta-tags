...
$sTableID = 'tbl_shantilab_metatags_keys_list';
...
if($lAdmin->EditAction())
{
	foreach($FIELDS as $ID=>$arFields)
	{
		...
		$DB->StartTransaction();
		$ID = IntVal($ID);
		$rsData = Shantilab\MetaTags\Key\DB::getById($ID);
		if($arData = $rsData->Fetch())
		{
			...
			$res = Shantilab\MetaTags\Key\DB::update($ID, $arData);
			if(!$res->isSuccess())
			{
...
if(($arID = $lAdmin->GroupAction()))
{
	...
	{
		$rsData = Shantilab\MetaTags\Key\DB::getList($listParams);
...
$columns = Shantilab\MetaTags\Key\KeysTable::getMap();
$defaultHeaders = array("ID" => 100,"CODE"=>200,"NAME"=>300,"VALUE"=>400,"ACTIVE"=>500,"SORT"=>600);
$headers = array();
...
while($arRes = $rsData->NavNext(true, "f_")):
	...
	/*pages*/
	$rs = Shantilab\MetaTags\Key\PagesTable::getList(array(
		"filter" => array("KEY_ID" => $f_ID)
	));
	$f_SHOW_ON = '';
	$f_SHOW_OFF = '';
	...
	$row->AddViewField("CODE", '<a href="'.Shantilab\MetaTags\Data::getPageName('edit_key').'?ID='.$f_ID.'&lang='.LANG.'">'.$f_CODE.'</a>');
	...
	$arActions[] = array(
		"ICON"=>"edit",
		"DEFAULT"=>true,
		"TEXT"=> GetMessage("KEY_EDIT_TITLE"),
		"ACTION"=>$lAdmin->ActionRedirect(Shantilab\MetaTags\Data::getPageName('edit_key')."?ID=".$f_ID.'&lang='.LANG)
	);
	...
	if($f_ACTIVE == "Y")
	{
		$arActions[] = array(
			"ICON" => "",
			"TEXT" => GetMessage("KEY_DEACTIVE_TITLE"),
	...
	else
	{
		$arActions[] = array(
			"ICON" => "",
			"TEXT" => GetMessage("KEY_ACTIVE_TITLE"),
	...
	$arActions[] = array(
		"ICON" => "copy",
		"TEXT" => GetMessage("KEY_COPY_TITLE"),
		"ACTION" => $lAdmin->ActionRedirect(Shantilab\MetaTags\Data::getPageName('edit_key')."?ID=".$f_ID.'&action=copy&lang='.LANG)
	);
...
<form name="find_form" method="get" action="<?echo $APPLICATION->GetCurPage();?>">
	...
	<tr>
		...
		<td>
			...
			echo SelectBoxFromArray("find_active", $arr, $find_active, GetMessage("KEY_ALL"), "");
			?>
		</td>
	</tr>
	<tr>
	...
	<tr>
		<td><?=$columns["CODE"]["title"]?>:</td>
		<td>
			<input type="text" name="find_code" size="47" value="<?echo htmlspecialcharsbx($find_code)?>">
		</td>
	</tr>
	<tr>
		<td><?=$columns["VALUE"]["title"]?>:</td>
		<td>
			<input type="text" name="find_value" size="47" value="<?echo htmlspecialcharsbx($find_value)?>">
		</td>
	</tr>
	<tr>
		...
		<td>
			...
			?>
			<?=SelectBoxFromArray("find_c_site", $arr, $find_c_site, GetMessage("KEY_ALL"))?>
		</td>
	</tr>
	<tr>
		...
		<td>
			...
			?>
			<?=SelectBoxFromArray("find_c_template", $arr, $find_c_template, GetMessage("KEY_ALL"), "")?>
		</td>
	</tr>
	<tr>
